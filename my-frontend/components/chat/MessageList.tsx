...
export const MessageList = ({ sendReadReceipt, sendDeleteMessage, onReply, sendReaction }: MessageListProps) => {
  ...
  // Close quick reaction bar on outside click
  useEffect(() => {
    ...
    <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 bg-gray-50 h-full relative" onClick={() => { setActiveMenuId(null); setEmojiPickerMsgId(null); }}>
      {[...messages].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()).map((msg) => {
        ...
        return (
          ...
          >
            {/* Quick Reaction Bar — shown when "React" is clicked from menu */}
            ...
            <div className={`flex gap-2 items-end ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    {msg.reply_to && (
                      <div 
                        ...
                        className={`text-xs p-2 rounded mb-1 border-l-2 cursor-pointer transition-colors ${
                          isMine ? 'bg-blue-700/30 border-blue-400 hover:bg-blue-700/50' : 'bg-gray-100 border-gray-400 hover:bg-gray-200'
                        }`}
                      >
              ...
              {!msg.deleted_for_everyone && msg.content !== 'This message was deleted' && (
                <div className={`relative opacity-0 group-hover/msg:opacity-100 transition-opacity flex-shrink-0 ${isMine ? 'order-first' : 'order-last'}`}>
                  ...
                  {activeMenuId === msg.id && (
                    <div className="absolute top-0 right-6 bg-white shadow-lg rounded-md border border-gray-100 text-sm z-[100] w-40 overflow-hidden" onClick={e => e.stopPropagation()}>
                      <button 
                      ...
                      <button 
                        className="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-700 flex items-center gap-2 border-t border-gray-100"
                  ...
                  {/* Emoji Picker Popup — now uses full EmojiPickerModal */}
